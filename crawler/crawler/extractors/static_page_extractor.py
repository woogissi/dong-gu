...
STATIC_UI_PHRASES = (
    "HOME", "Home", "로그인", "메뉴", "공유", "페이스북", "트위터",
    "카카오톡 공유", "URL 복사", "프린트", "게시물 검색", "게시판 목록",
    "이전글", "다음글", "첨부파일", "SNS 영역", "게시물 좌측으로 이동",
    "게시물 우측으로 이동", "이전 정지 시작 다음", "행사사진 More",
    "PDF 다운로드", "HWP 다운로드", "전체화면 보기",
)

STATIC_STUB_LINES = {
    "HOME", "Home", "TOP", "로그인", "메뉴", "공유", "페이스북", "트위터",
    "카카오톡 공유", "URL 복사", "프린트", "게시물 검색", "게시판 목록",
    "이전글", "다음글", "첨부파일", "More", "NOTICE", "PROGRAM", "SNS",
    "회원가입", "이용문의", "PDF 다운로드", "HWP 다운로드", "전체화면 보기",
}
...
GENERIC_BLOCK_TAGS = {
    ...
    "main", "ol", "p", "section", "table", "tbody", "td", "th", "thead",
    "tr", "ul",
}

MAIN_PAGE_EXCLUDE_SELECTORS = [
    ".notice", ".notice-list", ".latest", ".latest-list", ".board",
    ".board-list", ".program", ".program-list", ".gallery", ".photo",
    ".media", ".sns", ".login", ".member", ".popup", ".quick",
    "[class*='notice']", "[class*='latest']", "[class*='board']",
    "[class*='program']", "[class*='gallery']", "[class*='photo']",
    "[class*='sns']", "[class*='login']", "[class*='popup']",
    "[id*='notice']", "[id*='latest']", "[id*='board']",
    "[id*='program']", "[id*='gallery']", "[id*='sns']", "[id*='login']",
]

STATIC_NOISE_SELECTORS = [
    "header", "footer", "nav", "aside", "script", "style", "noscript",
    "form[action*='search']", ".breadcrumb", ".breadcrumbs", ".location",
    ".path", ".quick", ".quickMenu", ".quick-menu", ".pagination",
    ".paging", ".sns", ".sns-area", ".snsArea", ".share", ".share-area",
    ".shareArea", ".login", ".member", ".board-search", ".boardSearch",
    ".search-box", ".searchBox", ".board-util", ".boardUtil", ".board-list",
    ".boardList", ".btn-share", ".btn-print", "[class*='breadcrumb']",
    "[class*='location']", "[class*='quick']", "[class*='sns']",
    "[class*='share']", "[class*='login']", "[class*='member']",
    "[class*='paging']", "[class*='pagination']", "[class*='board-search']",
    "[class*='boardSearch']", "[class*='board-util']", "[class*='boardUtil']",
    "[class*='quick-menu']", "[id*='breadcrumb']", "[id*='location']",
    "[id*='quick']", "[id*='sns']", "[id*='share']", "[id*='login']",
    "[id*='paging']", "[id*='pagination']",
]

STATIC_INCLUDE_SELECTORS = [
    ".contents", ".content", ".sub-content", ".cont", ".intro", ".greeting",
    ".overview", ".vision", ".purpose", ".info", "#contents", "#content", "main",
]
...
class StaticPageExtractor(BaseExtractor):
    ...
    def remove_noise_nodes(self, node, is_main_page: bool = False) -> None:
        if not node:
            return
    ...
    def find_content_node(self, soup: BeautifulSoup, page_url: str | None = None):
        is_main_page = self.is_main_page_url(page_url or "")
        selectors = [*STATIC_INCLUDE_SELECTORS, "#board_skin"]
        ...
        for sel in selectors:
            ...
            if node:
                ...
                self.remove_noise_nodes(candidate, is_main_page=is_main_page)
                return candidate
    ...
    def render_table_lines(self, table: Tag) -> list[str]:
        ...
        for row in table.find_all("tr"):
            ...
            cell_texts = [
                ...
                if text and not self.is_generic_skip_line(text)
            ]
        ...
        for a in content_node.find_all("a", href=True):
            ...
            is_js_download = (
                ...
                and (
                    ...
                    or "down" in href_lower
                    or "attach" in href_lower
                )
            )

            if href_lower.startswith("javascript:") and not is_js_download:
                continue
            ...
            if parsed.netloc.lower() in SOCIAL_LINK_HOSTS:
                continue

            query = parsed.query.lower()
            path = parsed.path.lower()
            url_ext = Path(parsed.path).suffix.lower()
            mode = parse_qs(parsed.query).get("mode", [""])[0].lower()

            if (
                "mode=view" in query
                or "mod=view" in query
                or "articleno=" in query
                or "noticeno=" in query
            ):
                continue

            if (
                "mode=list" in query
                or "article.offset" in query
            ):
                continue
    ...
    def extract_static_page(self, source_type: str, page_url: str) -> dict:
        ...
        content_node = self.find_content_node(soup, page_url=canonical_page_url)

        raw_text_before_filter = (
            self.normalize_multiline_text(content_node.get_text("\n", strip=True))
            if content_node
            else ""
        )
        raw_text, text_filter_metadata = self.clean_static_text(
            raw_text_before_filter,
            is_main_page=is_main_page,
        )

        table_text_before_filter = self.extract_table_text(content_node)
        table_text, table_filter_metadata = self.clean_static_text(
            table_text_before_filter,
            is_main_page=is_main_page,
        )
        ...
        raw_doc = StaticPageRawDocument(
            doc_id=self.make_doc_id(canonical_page_url),
