...
DOMAIN_RULES: dict[str, dict[str, object]] = {
    ...
    "graduation": {
        ...
        "synonyms": {"졸업": ["졸업요건", "졸업학점", "졸업기준", "졸업인증제도", "학사정보 종합안내", "졸업최저이수학점"]},
        "intent_boost": "RAG",
        "category": "graduation",
        "source_boosts": ["academic_notice", "academic_support", "institution"],
    },
    "scholarship": {
...
ENTITY_ALIASES: dict[str, list[str]] = {
    ...
    "DOOR": ["도어"],
    # 학과 별칭
    "디자인조형학과": ["시각디자인과", "시각디자인학과"],
    "국가장학금": ["국장"],
}

DOMAIN_BLACKLIST = {"정보", "안내", "내용", "관련", "이름", "방법", "기간"}

# 동점 시 우선순위 (낮은 인덱스 = 높은 우선순위)
...
ENTITY_LEXICON: dict[str, list[str]] = {
    "학사": ["학사", "수강", "강의", "성적", "학점", "종강", "개강", "방학"],
    "장학": ["장학", "장학금", "국가장학", "근로장학"],
    "등록": ["등록", "등록금", "납부", "고지서"],
    "졸업": ["졸업", "졸업요건", "졸업학점", "논문", "졸업식", "학위수여식"],
    "휴학": ["휴학", "군휴학", "일반휴학"],
    "복학": ["복학", "복학신청"],
    "수강": ["수강", "수강신청", "정정"],
    "기숙사": ["기숙사", "생활관", "긱사", "효민생활관", "행복기숙사"],
    "비교과": ["비교과", "비교과프로그램", "프로그램", "특강", "오리엔테이션"],
    "국제": ["국제", "교환학생", "어학"],
    "신입생": ["신입생", "새내기"],
    "재학생": ["재학생"],
    "복학생": ["복학생"],
...
ENTITY_SYNONYMS: dict[str, tuple[str, ...]] = {
    ...
    "수강신청": ("강의신청",),
    "교환학생": ("국제교류",),
    # 종강 → 방학 시작일 (scheduleList 검색 보강)
    "종강": ("방학 시작일", "하계방학", "동계방학"),
    "종강일": ("방학 시작일", "하계방학 시작일", "동계방학 시작일"),
    # 졸업식 ↔ 학위수여식 (명시적 양방향)
    "졸업식": ("학위수여식",),
    "학위수여식": ("졸업식",),
}
...
REWRITE_ENTITY_SURFACES: dict[str, tuple[str, ...]] = {
    ...
    "수강": ("수강", "강의"),
    "학사공지": ("학사공지",),
    "졸업": ("졸업", "졸업요건", "졸업학점", "졸업기준", "졸업인증제도", "학사정보 종합안내", "논문"),
    "기숙사": ("기숙사",),
    "생활관": ("생활관",),
}
