...
from rag.retrieval.retriever import merge_retrieval_candidates, retrieve_documents
...
def fetch_queries(cur: Any, limit: int) -> list[dict[str, Any]]:
    cur.execute(
        """
        SELECT DISTINCT ON (q.request_id)
            q.request_id,
            q.question,
            q.intent_type,
            q.created_at,
            ...
            rl.fallback_used
        FROM query_logs q
        LEFT JOIN retrieval_logs rl ON rl.request_id = q.request_id
        WHERE q.question IS NOT NULL
          AND length(trim(q.question)) > 0
        ORDER BY q.request_id, rl.created_at DESC NULLS LAST, q.created_at DESC
        LIMIT %s
        """,
        (limit,),
    )
...
def precise_terms(terms: list[str]) -> list[str]:
    ...
    for term in terms:
        ...
        if normalized in GENERIC_TERMS:
            continue
        if normalized not in strong:
            strong.append(normalized)
    return strong[:8]


def precise_probe(cur: Any, terms: list[str], limit: int = 8) -> list[dict[str, Any]]:
    ...
    cur.execute(
        ...
        SELECT
            ...
            d.source_type,
            d.department,
            left(c.content, 240) AS content_preview
        FROM chunks c
        JOIN documents d ON d.doc_id = c.doc_id
        WHERE coalesce(d.title, '') ILIKE ANY(%s)
           OR coalesce(c.section_title, '') ILIKE ANY(%s)
           OR coalesce(c.content, '') ILIKE ANY(%s)
        ORDER BY d.published_at DESC NULLS LAST, c.chunk_id ASC
        LIMIT %s
        """,
...
def run_retrieval(request: Any, *, retrieval_mode: str) -> list[Any]:
    previous = os.getenv("RETRIEVAL_MODE")
    ...
    try:
        ...
        if previous is None:
            ...
            os.environ["RETRIEVAL_MODE"] = previous


def fuse_candidates(lexical_docs: list[Any], vector_docs: list[Any], *, mode: str, beta: float | None = None) -> list[Any]:
    previous = {
        "HYBRID_SCORE_MODE": os.getenv("HYBRID_SCORE_MODE"),
        "HYBRID_SRRF_BETA": os.getenv("HYBRID_SRRF_BETA"),
    }
    os.environ["HYBRID_SCORE_MODE"] = mode
    if beta is not None:
        os.environ["HYBRID_SRRF_BETA"] = str(beta)
    try:
        return merge_retrieval_candidates(lexical_docs, vector_docs)
    finally:
...
def score_setting(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "avg_evidence_rank": rounded_mean(evidence_ranks),
...
def rounded_mean(values: Any) -> float | None:
    present = [value for value in values if value is not None and math.isfinite(float(value))]
    return round(mean(present), 3) if present else None


def choose_recommendation(summary: dict[str, dict[str, Any]]) -> dict[str, Any]:
    candidates = {name: metrics for name, metrics in summary.items() if name.startswith("srrf_beta_")}
    has_evidence_hit = any((metrics.get("hit_at_10") or 0.0) > 0 for metrics in candidates.values())

    def key(item: tuple[str, dict[str, Any]]) -> tuple[float, float, float, float]:
        _, metrics = item
        if not has_evidence_hit:
            return (
                -metric_value(metrics, "top1_changed_vs_rrf", 999.0),
                -metric_value(metrics, "avg_rank_delta_top10_vs_rrf", 999.0),
                metrics.get("avg_overlap_top10_vs_rrf") or 0.0,
                -metric_value(metrics, "top1_changed_vs_weighted", 999.0),
            )
        return (
            metrics.get("hit_at_5") or 0.0,
            -metric_value(metrics, "avg_evidence_rank", 999.0),
            metrics.get("avg_overlap_top10_vs_rrf") or 0.0,
            -metric_value(metrics, "top1_changed_vs_rrf", 999.0),
        )
    ...
    return {
        ...
        "reason": (
            "hit@5, avg evidence rank, then RRF stability."
            if has_evidence_hit
            else "No setting placed precise evidence in top10, so selected by RRF stability: top1 preservation, then lower top10 rank delta."
        ),
...
def main() -> None:
    ...
    parser.add_argument("--output", default="/app/rag/evaluation/results/srrf_tuning_report.json")
    ...
    with open_connection() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            for index, case in enumerate(fetch_queries(cur, args.limit), start=1):
                query = case["question"]
                request, query_analysis = build_request(preprocessor, embedder, query, args.top_k)
                evidence_rows = precise_probe(cur, query_analysis["terms"])
                ...
                lexical_docs = run_retrieval(request.model_copy(update={"strategy": "keyword"}), retrieval_mode="lexical")
                vector_docs = run_retrieval(request.model_copy(update={"strategy": "vector"}), retrieval_mode="vector")
                docs_by_setting = {
                    name: fuse_candidates(lexical_docs, vector_docs, mode=mode, beta=beta)
                ...
                case_results.append(
                    {
                        ...
                        "precise_terms": precise_terms(query_analysis["terms"]),
                        "evidence_exists": bool(evidence_rows),
